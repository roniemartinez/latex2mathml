...
def test_matrix():
    ...
    assert [r'\matrix', '{', 'a', '&', 'b', r'\\', 'c', '&', 'd', '}'] == aggregate(TexSoup.TexSoup(s))


def test_matrix_with_alignment():
    s = r'\begin{matrix*}[r]a & b \\ c & d \end{matrix*}'
    assert [r'\matrix*', 'r', '{', 'a', '&', 'b', r'\\', 'c', '&', 'd', '}'] == aggregate(TexSoup.TexSoup(s))
...
def test_simple_array():
    s = r'\begin{array}{cc} 1 & 2 \\ 3 & 4 \end{array}'''
    assert [r'\array', 'cc', '{', '1', '&', '2', r'\\', '3', '&', '4', '}'] == aggregate(TexSoup.TexSoup(s))
